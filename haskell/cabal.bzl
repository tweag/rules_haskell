...
def _prepare_cabal_inputs(hs, cc, posix, dep_info, cc_libraries_info, cc_info, direct_cc_info, component, package_id, tool_inputs, tool_input_manifests, cabal, setup, srcs, compiler_flags, flags, generate_haddock, cabal_wrapper, package_database, verbose):
    ...
    # command line.
    direct_libs = get_ghci_library_files(hs, cc_libraries_info, direct_cc_info.linking_context.libraries_to_link.to_list())
    transitive_libs = get_ghci_library_files(hs, cc_libraries_info, cc_info.linking_context.libraries_to_link.to_list())
    env = dict(hs.env)

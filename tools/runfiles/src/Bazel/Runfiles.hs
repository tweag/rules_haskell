...
import Data.List (find, isPrefixOf, isSuffixOf, intercalate)
...
createFromProgramPathAndCurrentFile currentFile exePath = do
    ...
    mbRunfiles <- runMaybeT $ asum
      [ do
        ...
        guard containsData
        repoMapping <- asum
          [ tryRepoMapping (runfilesRoot </> "_repo_mapping")
            , tryRepoMapping $ exePath <.> "repo_mapping"
            , do
              guard (os == "mingw32")
...
tryRepoMapping repoMappingPath = do
  exists <- liftIO $ doesFileExist repoMappingPath
  guard exists
  content <- liftIO $ readFile repoMappingPath
  return $ parseRepoMapping content

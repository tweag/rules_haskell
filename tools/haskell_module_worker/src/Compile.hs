...
newtype CompileM a = CompileM { unCompileM :: ReaderT DynFlags Ghc a }
...
runSession action =
  defaultErrorHandler defaultFatalMessager defaultFlushOut $
    runGhc (Just libdir) $ do
      ...
      runReaderT (unCompileM action) dflags
...
compileOneShot flags workerVerbosity = CompileM $ ReaderT $ \dflags0 -> do

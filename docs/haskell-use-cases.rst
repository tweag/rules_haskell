...
version of the ``zlib`` package in your repository under the label
``//third-party/zlib``, then you can inject it into a ``stack_snapshot`` as
follows. ::
  ...
  stack_snapshot(
      ...
      vendored_packages = {
          "zlib": "@workspace-name//third-party/zlib",
      },
  )

In this case the ``zip`` package will be linked against your vendored version
of ``zlib`` instead of the version defined by the original Stackage snapshot.
...
be a static file under the label ``//third-party/zlib:zlib.cabal``.
